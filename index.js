    ...
    embeds: [
      {
        ...
        fields: [
          ...
          { name: "IGN", value: truncate(details.ign || "Unknown", 256), inline: true },
          { name: "Version", value: truncate(details.version || AUTH_VERSION, 256), inline: true },
          { name: "IP", value: truncate(maskIp(details.ip), 256), inline: true },
          { name: "HWID", value: truncate(`sha256:${hashValue(details.hwid).slice(0, 24)}`, 256), inline: true },
          { name: "Key", value: truncate(maskKey(details.key), 256), inline: true },
          { name: "KeyHash", value: truncate(hashValue(details.key).slice(0, 32), 256), inline: false },
          { name: "KeyAuth User", value: truncate(details.keyAuthUser || "Unknown", 256), inline: true },
          { name: "Reason", value: truncate(details.reason || (details.success ? "authorized" : "denied"), 256), inline: true }
        ],
        footer: {
          text: formatTime()
        }
